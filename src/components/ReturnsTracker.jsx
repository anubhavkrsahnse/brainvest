    ...
    <div className="dashboard" id="returns">
      ...
      <div className="kite-summary">
        <div className="kite-stat">
          ...
          <span className="k-value">{inr(totals.invested)}</span>
        </div>
        <div className="kite-stat">
          <span className="k-label">Current value</span>
          <span className="k-value">{inr(totals.current)}</span>
        </div>
        <div className="kite-stat">
          <span className="k-label">Day's P&amp;L</span>
          <span className={`k-value ${cls(totals.dayPnl)}`}>{signed(totals.dayPnl)}</span>
        </div>
        <div className="kite-stat">
          <span className="k-label">Total P&amp;L</span>
          <span className={`k-value ${cls(totals.pnl)}`}>
            {signed(totals.pnl)} <small className={`k-chip ${cls(totals.pnl)}`}>{totalPct >= 0 ? '+' : ''}{totalPct.toFixed(2)}%</small>
          </span>
        </div>
      </div>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id}>
                ...
                <td className="num">{r.invested ? inr(r.current) : '—'}</td>
                <td className={`num ${cls(r.pnl)}`}>{r.invested ? signed(r.pnl) : '—'}</td>
